 ...
 */
package org.l2explorer.actions;
...
import org.l2explorer.data.GameDataName;
import org.l2explorer.util.DebugUtil;
import org.l2explorer.xml.CryptVersionParser;
import org.l2explorer.xml.Descriptor;
import org.l2explorer.xml.DescriptorParser;
import org.l2explorer.xml.DescriptorReader;
...
public class OpenDat extends ActionTask
{
	...
	public OpenDat(L2ExplorerDat l2ClientDat, String structureChronicle, File file)
	{
		super(l2ClientDat);
	...
	public static ByteBuffer decrypt(File file, Collection<DatCrypter> decryptors, boolean mass) throws Exception
	{
		...
		if (!file.exists())
		{
			if (!mass)
			{
				L2ExplorerDat.addLogConsole("File " + fileName + " does not exist.", true);
			}
			return null;
		}
		...
		if (!file.canRead())
		{
			if (!mass)
			{
				L2ExplorerDat.addLogConsole("Unable to read " + fileName + " file.", true);
			}
			return null;
		}
		...
		if (fis.available() < 28)
		{
			if (!mass)
			{
				L2ExplorerDat.addLogConsole("The file " + fileName + " is too small.", true);
			}
			fis.close();
			return null;
		}
		...
		if (!header.matches("Lineage2Ver(\\d{3})"))
		{
			if (!mass)
			{
				L2ExplorerDat.addLogConsole("File " + fileName + " not encrypted. Skip decrypt.", true);
			}
			crypt = false;
		}
		...
		if (crypt)
		{
			...
			if (!mass)
			{
				L2ExplorerDat.addLogConsole("File " + fileName + " encrypted. " + header + " decrypt ...", true);
			}
			...
			if (crypter == null)
			{
				if (!mass)
				{
					L2ExplorerDat.addLogConsole("Error decrypt " + fileName + " file.", true);
				}
				return null;
			}
			...
			if (!mass)
			{
				L2ExplorerDat.addLogConsole("Decrypt " + fileName + " file successfully by " + crypter.getName() + " decrypter.", true);
			}
		}
		else
		{
			...
			catch (IOException exc)
			{
				if (!mass)
				{
					L2ExplorerDat.addLogConsole("Error reading" + fileName + "  file.", true);
				}
			}
		}
		return buffer;
	...
	public static String start(ActionTask actionTask, double weight, String structureChronicle, File file, boolean mass) throws Exception
	{
		...
		else if (fileName.endsWith(".unr") || fileName.endsWith(".ukx") || fileName.endsWith(".usx") || fileName.endsWith(".uax") || fileName.endsWith(".uix"))
		{
			...
			if (!mass)
			{
				L2ExplorerDat.addLogConsole("Loaded raw binary file (no descriptor): " + fileName, true);
			}
		...
		else
		{
			if ((crypter == null) || !crypter.isUseStructure())
			{
				if (!mass)
				{
					L2ExplorerDat.addLogConsole("Unknown file " + fileName + " type!", true);
				}
				return null;
			}
			...
			if (!mass)
			{
				L2ExplorerDat.addLogConsole("Read the file structure ...", true);
			}
			...
			if (text == null)
			{
				if (!mass)
				{
					L2ExplorerDat.addLogConsole("Structure is not found in the directory: " + structureChronicle + " file: " + fileName, true);
				}
				return null;
			}
		}
		...
		if (!mass)
		{
			L2ExplorerDat.addLogConsole("Completed.", true);
		}
