...
public class MassRecryptor extends ActionTask
{
	...
	public MassRecryptor(L2ExplorerDat l2ClientDat, String path)
	{
		super(l2ClientDat);
		_path = path;
	}
	...
	{
		L2ExplorerDat.addLogConsole("Mass recrypt by path [" + _path + "]", true);
		...
		{
			L2ExplorerDat.addLogConsole("Cannot create backup directory [" + backupDirPath + "].", true);
			return;
		}
		...
		L2ExplorerDat.addLogConsole("---------------------------------------", true);
		...
		for (File file : files)
		{
			...
			READ:
			{
				try (FileInputStream fis = new FileInputStream(file))
				{
					if (fis.available() < 28)
					{
						L2ExplorerDat.addLogConsole("The file " + file.getName() + " is too small.", true);
						break READ;
					}
					...
					{
						L2ExplorerDat.addLogConsole("File " + file.getName() + " not encrypted. Skip decrypt.", true);
						break READ;
					}
					...
					L2ExplorerDat.addLogConsole(file.getName() + " change crypt by " + encrypter.getName() + " encryptor success.", true);
				}
				catch (Exception e)
				{
					L2ExplorerDat.addLogConsole(file.getName() + " change crypt by " + encrypter.getName() + " encryptor failed!", true);
				}
				finally
				{
