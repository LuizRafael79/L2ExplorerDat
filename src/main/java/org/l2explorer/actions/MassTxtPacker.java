...
import org.l2explorer.clientcryptor.crypt.RSADatCrypter;
...
import org.l2explorer.xml.DescriptorWriter;
...
public class MassTxtPacker extends ActionTask
{
	...
	public MassTxtPacker(L2ExplorerDat l2ClientDat, String chronicle, String path)
	{
		super(l2ClientDat);
	...
	{
		L2ExplorerDat.addLogConsole("Mass packer with using " + _chronicle + " chronicles by path [" + _path + "]", true);
		
		final File baseDir = new File(_path);
		if (!baseDir.exists())
		{
			L2ExplorerDat.addLogConsole("Directory [" + _path + "] does not exists.", true);
			return;
		}
		...
		{
			L2ExplorerDat.addLogConsole("Selected encryptor not RSA! Please select RSA encryptor.", true);
			return;
		}
		
		if (!encrypter.isEncrypt())
		{
			L2ExplorerDat.addLogConsole("Selected encryptor does not have encrypt RSA key.", true);
			return;
		}
		...
		{
			L2ExplorerDat.addLogConsole("Cannot create directory [" + packDir + "].", true);
			return;
		}
		...
		{
			L2ExplorerDat.addLogConsole("Directory [" + _path + "] is empty.", true);
			return;
		}
		...
		L2ExplorerDat.addLogConsole("Completed. Elapsed ".concat(String.valueOf(diffTime)).concat(" sec"), true);
	}
	...
	private static void pack(ActionTask actionTask, double weight, String chronicle, DatCrypter encrypter, File file, File packDir)
	{
		...
		L2ExplorerDat.addLogConsole("Start packing [" + file.getName() + "]...", true);
		...
		try
		{
			...
			if (file.getName().endsWith(".dat") || file.getName().endsWith(".txt"))
			{
				...
				{
					L2ExplorerDat.addLogConsole("Not found the structure of the file: " + file.getName(), true);
				}
			}
			...
			{
				L2ExplorerDat.addLogConsole("Unknown file [" + file.getName() + "] type!", true);
				shouldContinue = false;
			}
			...
			if (shouldContinue && (buff != null))
			{
				try
				{
					...
					L2ExplorerDat.addLogConsole("Success packed [" + file.getName() + "]", true);
				}
				catch (Exception e)
				{
